...
class AboutSymbols < Neo::Koan
  ...
  def test_identical_symbols_are_a_single_internal_object
    ...
    assert_equal true, symbol1           == symbol2
    assert_equal true, symbol1.object_id == symbol2.object_id #Symbols are single internal objects
  end

  def test_method_names_become_symbols
    symbols_as_strings = Symbol.all_symbols.map { |x| x.to_s }  #Converts each symbol to it's string representation | Symbol.all_symbols returns an array of all symbols. | map iterates over array and converts each element into string repreesentation
